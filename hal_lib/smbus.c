...
long read_byte(Bus *bus, uint8_t addr) {
    bus->ret = HAL_I2C_Master_Receive(&(bus->hi2c), (addr << 1) | 1, bus->buf, 1, HAL_MAX_DELAY);
    check_error(bus);
}

void write_byte(Bus *bus, uint8_t addr, uint8_t data) {
    bus->buf[0] = data;
    bus->ret = HAL_I2C_Master_Transmit(&(bus->hi2c), addr << 1, bus->buf, 1, HAL_MAX_DELAY);
    check_error(bus);
}
...
void write_byte_data(Bus *bus, uint8_t addr, char cmd, uint8_t data) {
    ...
    bus->ret = HAL_I2C_Master_Transmit(&(bus->hi2c), addr << 1, bus->buf, 2, HAL_MAX_DELAY);
    check_error(bus);
}
...
long read_word_data(Bus *bus, uint8_t addr, char cmd) {
    ...
    //reads from address sent above
    bus->ret = HAL_I2C_Master_Receive(&(bus->hi2c), (addr << 1) | 1, bus->buf, 2, HAL_MAX_DELAY);
    check_error(bus);
...
void write_word_data(Bus *bus, uint8_t addr, char cmd, uint16_t data) {
    ...
    bus->ret = HAL_I2C_Master_Transmit(&(bus->hi2c), addr << 1, bus->buf, 3, HAL_MAX_DELAY);
...
int check_error(Bus *bus) {
    ...
    HAL_UART_Transmit(&(bus->huart), bus->buf, strlen((char*)bus->buf), HAL_MAX_DELAY);
    HAL_Delay(10);
        return false;
    }
    return true;
