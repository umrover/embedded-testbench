...
void CH_process_received(I2CBus* i2c_bus, Motor *motor) {
	switch(i2c_bus->operation) {
	case OFF: set_motor_speed(motor, 0.0f); return;
	case ON: return;
	case OPEN:
	case OPEN_PLUS: motor->mode = 0x00; memcpy(&(motor->desired_speed), i2c_bus->buffer, 4); return;
	case CLOSED:
	case CLOSED_PLUS: motor->mode = 0xFF; memcpy(&(motor->gains->kF), i2c_bus->buffer, 4); memcpy(&(motor->desired_angle),i2c_bus->buffer+4,4); return;
	case CONFIG_PWM: {
		int max = 0;
		memcpy(&(max),i2c_bus->buffer,2);
		motor->speed_limit = (float)(max)/100; return; //UPDATED
	}
	case CONFIG_K: memcpy(&(motor->gains->kP),i2c_bus->buffer,4); memcpy(&(motor->gains->kI),i2c_bus->buffer+4,4); memcpy(&(motor->gains->kD),i2c_bus->buffer+8,4); return;
	case QUAD_ENC: return;
	case ADJUST: memcpy(&(motor->encoder->raw), i2c_bus->buffer, 4); return;
	case ABS_ENC:
	case LIMIT:
	case CALIBRATED: return;
	case LIMIT_ON: memcpy(&(motor->limit_enabled), i2c_bus->buffer, 1);
	case UNKNOWN: return;
	}
}
...
void CH_prepare_send(I2CBus* i2c_bus, Motor *motor) {
	...
	case ON:
	case OPEN: return;
	case OPEN_PLUS: memcpy(i2c_bus->buffer, &(motor->encoder->raw), 4); return;
	case CLOSED: return;
	case CLOSED_PLUS: memcpy(i2c_bus->buffer, &(motor->encoder->raw), 4); return;
	case CONFIG_PWM:
	case CONFIG_K: return;
	case QUAD_ENC: memcpy(i2c_bus->buffer, &(motor->encoder->raw), 4); return;
	case ADJUST: return;
	// case ABS_ENC: memcpy(i2c_bus->buffer, &(motor->abs_enc_value), 4); return;
	case ABS_ENC: return; // TODO add support for abs encoder
	// case LIMIT: memcpy(i2c_bus->buffer, &(motor->limit), 1); return;
	case LIMIT: return; // TODO add cases for both limits
	case CALIBRATED: memcpy(i2c_bus->buffer, &(motor->calibrated), 1); return;
	case LIMIT_ON:
	case UNKNOWN: return;
	}
}
