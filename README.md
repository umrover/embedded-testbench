...
[Link](https://docs.google.com/document/d/1O2tBbl5UmaS5ZpxchtgLlV1p0EEsiNUQgIRuh8l4p64/edit) to the integration testing doc
...
#### On the logic analyzer the UART line is noisy 
Determine what turns on to make this line noisy and take some measure to dampen the noise or something. \
In our case we were running the peristaltic pump off of a MOSFET so instead we switched it electrically to an HBridge

#### Other 
We also grounded the CTS pin and that was helpful 

### The LCM messages from the science nucleo are slow to update
Decrease the async sleep time, it's probably updating slower than the science nucleo UART is sending messages 

### LCM messages aren't showing up on the base_station 
Prefix both the base_station gui and science-bridge programs with the LCM_DEFAULT_URL="udpm://239.255.76.67:7667?ttl=255". \
You have may have to stop the base_station gui/ science nucleo ansible scripts to do this
...
- [ ] Implement a restart command for the i2c bus of the spectral sensors, if they get unplugged the messages get garbled 
