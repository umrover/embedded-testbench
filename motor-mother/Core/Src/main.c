...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t TransferDirection,
	...
	if (TransferDirection == I2C_DIRECTION_TRANSMIT) {
		HAL_I2C_Slave_Seq_Receive_IT(i2c_bus->i2c_bus_handle, i2c_bus->buffer,
				1, I2C_LAST_FRAME);
		...
		if (bytes_to_send != 0) {
			HAL_I2C_Slave_Seq_Transmit_IT(i2c_bus->i2c_bus_handle,
					i2c_bus->buffer, bytes_to_send, I2C_LAST_FRAME);
		}
	}
...
void HAL_I2C_SlaveRxCpltCallback(I2C_HandleTypeDef *hi2c) {
	if (i2c_bus->operation == UNKNOWN) {
		...
		if (bytes_to_recieve != 0) {
			HAL_I2C_Slave_Seq_Receive_IT(i2c_bus->i2c_bus_handle,
					i2c_bus->buffer, bytes_to_recieve, I2C_LAST_FRAME);
		} else {
			if (i2c_bus->motor_id < NUM_MOTORS) {
...
int main(void) {
	...
	hbridge_backward_pins[0] = new_pin(MOTOR_NDIR_0_GPIO_Port,
			MOTOR_NDIR_0_Pin);
	hbridge_backward_pins[1] = new_pin(MOTOR_NDIR_1_GPIO_Port,
			MOTOR_NDIR_1_Pin);
	hbridge_backward_pins[2] = new_pin(MOTOR_NDIR_2_GPIO_Port,
			MOTOR_NDIR_2_Pin);
	hbridge_backward_pins[3] = new_pin(MOTOR_NDIR_3_GPIO_Port,
			MOTOR_NDIR_3_Pin);
	hbridge_backward_pins[4] = new_pin(MOTOR_NDIR_4_GPIO_Port,
			MOTOR_NDIR_4_Pin);
	hbridge_backward_pins[5] = new_pin(MOTOR_NDIR_5_GPIO_Port,
			MOTOR_NDIR_5_Pin);

	hbridges[0] = new_hbridge(&htim4, TIM_CHANNEL_1, &(TIM4->CCR1),
			&(TIM4->ARR), hbridge_forward_pins[0], hbridge_backward_pins[0]);
	hbridges[1] = new_hbridge(&htim4, TIM_CHANNEL_2, &(TIM4->CCR2),
			&(TIM4->ARR), hbridge_forward_pins[1], hbridge_backward_pins[1]);
	hbridges[2] = new_hbridge(&htim4, TIM_CHANNEL_3, &(TIM4->CCR3),
			&(TIM4->ARR), hbridge_forward_pins[2], hbridge_backward_pins[2]);
	hbridges[3] = new_hbridge(&htim4, TIM_CHANNEL_4, &(TIM4->CCR4),
			&(TIM4->ARR), hbridge_forward_pins[3], hbridge_backward_pins[3]);
	hbridges[4] = new_hbridge(&htim15, TIM_CHANNEL_1, &(TIM15->CCR1),
			&(TIM15->ARR), hbridge_forward_pins[4], hbridge_backward_pins[4]);
	hbridges[5] = new_hbridge(&htim15, TIM_CHANNEL_2, &(TIM15->CCR2),
			&(TIM15->ARR), hbridge_forward_pins[5], hbridge_backward_pins[5]);
...
//	quad_encoders[5] = new_quad_encoder(&htimX, TIMX);
	...
	for (size_t i = 0; i < NUM_MOTORS; ++i) {
		motors[i] = new_motor(hbridges[i], forward_limit_switches[i],
				backward_limit_switches[i], quad_encoders[i], controls[i]);
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOA, MOTOR_DIR_4_Pin | MOTOR_NDIR_4_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOC, MOTOR_DIR_5_Pin | MOTOR_NDIR_5_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB,
			MOTOR_DIR_0_Pin | MOTOR_NDIR_0_Pin | MOTOR_DIR_1_Pin
					| MOTOR_NDIR_1_Pin | DEBUG_LED_1_Pin | DEBUG_LED_0_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOD,
			MOTOR_DIR_2_Pin | MOTOR_NDIR_2_Pin | MOTOR_DIR_3_Pin
					| MOTOR_NDIR_3_Pin | DEBUG_LED_3_Pin | DEBUG_LED_2_Pin,
			GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = MOTOR_DIR_0_Pin | MOTOR_NDIR_0_Pin | MOTOR_DIR_1_Pin
			| MOTOR_NDIR_1_Pin | DEBUG_LED_1_Pin | DEBUG_LED_0_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	...
	GPIO_InitStruct.Pin = MOTOR_DIR_2_Pin | MOTOR_NDIR_2_Pin | MOTOR_DIR_3_Pin
			| MOTOR_NDIR_3_Pin | DEBUG_LED_3_Pin | DEBUG_LED_2_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	...
	GPIO_InitStruct.Pin = LIMIT_A_0_Pin | LIMIT_B_0_Pin | LIMIT_A_1_Pin
			| LIMIT_B_1_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
	...
	GPIO_InitStruct.Pin = LIMIT_A_2_Pin | LIMIT_B_2_Pin | LIMIT_A_3_Pin
			| LIMIT_B_3_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
