...
uint8_t virtual_read(uint8_t v_reg) {
	...
	status = read_byte_data(i2c_bus_triad, DEVICE_SLAVE_ADDRESS, I2C_AS72XX_SLAVE_STATUS_REG);

	if ((status & I2C_AS72XX_SLAVE_RX_VALID) != 0) {
		// d = nucleo_byte_read(I2C_AS72XX_SLAVE_READ_REG);
		d = read_byte_data(i2c_bus_triad, DEVICE_SLAVE_ADDRESS, I2C_AS72XX_SLAVE_READ_REG);
	}
	...
	write_byte_data(i2c_bus_triad, DEVICE_SLAVE_ADDRESS, I2C_AS72XX_SLAVE_WRITE_REG, v_reg);
...
void virtual_write(uint8_t v_reg, uint8_t data) {
	...
	write_byte_data(i2c_bus, DEVICE_SLAVE_ADDRESS, I2C_AS72XX_SLAVE_WRITE_REG, (v_reg | 1 << 7));
...
//		status = nucleo_byte_read(I2C_AS72XX_SLAVE_STATUS_REG);
	...
	write_byte_data(i2c_bus_triad, DEVICE_SLAVE_ADDRESS, I2C_AS72XX_SLAVE_WRITE_REG, data);
}
...
void send_triad_data(uint16_t *data, UART_HandleTypeDef * huart){
	...
	// 155 - total length of output string $TRIAD,x,y,z...
	char string[155] = "";
