...
long read_byte(SMBus *smbus, uint8_t addr) {
    if (!smbus->DMA){
        ...
        smbus->ret = HAL_I2C_Master_Receive_DMA(smbus->i2c, (addr << 1) | 1, smbus->buf, 1, HAL_MAX_DELAY);
    }
    _check_error(smbus);
    return smbus->buf[0];
}
...
void write_byte(SMBus *smbus, uint8_t addr, uint8_t data) {
    ...
    if (!smbus->DMA) {
        ...
        smbus->ret = HAL_I2C_Master_Transmit_DMA(smbus->i2c, addr << 1, smbus->buf, 1, HAL_MAX_DELAY);
    }
    _check_error(smbus);
}
...
void write_byte_data(SMBus *smbus, uint8_t addr, char cmd, uint8_t data) {
    ...
    if (!smbus->DMA) {
        ...
        smbus->ret = HAL_I2C_Master_Transmit_DMA(smbus->i2c, addr << 1, smbus->buf, 2, HAL_MAX_DELAY);
    }
    _check_error(smbus);
}
...
long read_word_data(SMBus *smbus, uint8_t addr, char cmd) {
    ...
    if (!smbus->DMA){
        ...
        smbus->ret = HAL_I2C_Master_Receive_DMA(smbus->i2c, (addr << 1) | 1, smbus->buf, 2, HAL_MAX_DELAY);
    }
    _check_error(smbus);
...
void write_word_data(SMBus *smbus, uint8_t addr, char cmd, uint16_t data) {
    ...
    if(!smbus->DMA) {
       ...
       smbus->ret = HAL_I2C_Master_Transmit_DMA(smbus->i2c, addr << 1, smbus->buf, 3, HAL_MAX_DELAY); 
    }
